 ...
 * v11.2 - Compact Mobile UI & Plus Icon Update.
 */
...
const StatusToast = ({ toast, onClose }) => {
    ...
    return (
        <div className={`absolute top-6 left-1/2 -translate-x-1/2 z-[60] px-4 py-2 md:px-6 md:py-3 rounded-full shadow-xl backdrop-blur flex items-center gap-2 md:gap-3 animate-in slide-in-from-top-4 fade-in duration-300 max-w-[90%] w-auto mx-auto ${isError ? 'bg-red-500 text-white' : 'bg-emerald-600 text-white'}`}>
...
export default function IFStudio() {
  ...
  // Null means no tab is active (fullscreen image view)
  ...
  useEffect(() => {
      if (window.innerWidth < 768) {
          ...
          setActiveTab(null); // Start with full view on mobile
      }
  }, []);
  ...
  const handleCanvasClick = () => {
      if (window.innerWidth < 768) {
          setActiveTab(null); // Close menus on canvas click
      }
  };
  ...
  const handleStart = (e) => {
    if (!imageSrc) return;
    if (step !== 'crop') {
        // In edit mode, clicking canvas also closes menus
        handleCanvasClick();
        return; 
    }
    setIsDragging(true);
              ...
              <>
                ...
                {mode === 'dots' && (
                    <div className="mb-6 p-4 bg-gray-50 rounded-2xl border border-gray-200">
                        <span className="text-[10px] uppercase font-bold text-gray-400 mb-3 block tracking-wider">Shape</span>
          ...
          case 'tune': return (
              <>
                 ...
                 <div className="flex items-center justify-between mt-4 border-t border-gray-100 pt-4">
                    <span className="text-xs font-bold text-gray-400 uppercase">Reset All</span>
                    <button onClick={resetPatternSettings} className="p-2 bg-gray-100 rounded-lg text-gray-600 hover:bg-gray-200"><RotateCcw size={16}/></button>
                 </div>
              </>
          );
          case 'color': return (
          ...
          case 'print': return (
              <div className="space-y-6">
                         ...
                         <div className="p-4 bg-blue-50 rounded-xl border border-blue-100 animate-in fade-in">
                             <div className="flex justify-between items-center mb-2">
                                ...
                                <input 
                                    ...
                                    className="w-20 p-1 text-right border border-gray-300 rounded text-sm"
                                />
                             </div>
                             <Slider 
                                label="Min Thickness" 
                 ...
                 <div className="pt-4 border-t border-gray-100">
                    <span className="text-xs font-bold text-gray-400 uppercase tracking-wider block mb-3">Export File</span>
                    <div className="grid grid-cols-2 gap-3">
                        <button onClick={downloadSVG} disabled={!imageSrc} className="flex items-center justify-center py-3 bg-blue-50 hover:bg-blue-100 text-blue-600 rounded-xl font-bold text-sm border border-blue-200 shadow-sm disabled:opacity-50 transition-colors">
                             <Download size={16} className="mr-2" /> SVG
                        </button>
                        <button onClick={downloadPNG} disabled={!imageSrc} className="flex items-center justify-center py-3 bg-blue-50 hover:bg-blue-100 text-blue-600 rounded-xl font-bold text-sm border border-blue-200 shadow-sm disabled:opacity-50 transition-colors">
  ...
  return (
    <div className="flex h-screen w-full bg-gray-50 text-slate-800 font-sans overflow-hidden" style={{ touchAction: 'none' }}>
      ...
      <div className={`hidden md:flex fixed inset-y-0 left-0 w-96 bg-white border-r border-gray-200 flex-col overflow-hidden shadow-xl z-40`}>
        <div className="p-6 border-b border-gray-100 bg-white sticky top-0 z-10 shrink-0 flex items-center gap-3">
      ...
      {/* CANVAS AREA */}
      <div 
        className="flex-1 relative flex flex-col h-full overflow-hidden md:pl-96"
        ref={containerRef}
      >
        {/* Top Bar Mobile */}
        <div className="md:hidden h-12 bg-white border-b border-gray-200 flex items-center justify-between px-4 shrink-0 z-30 relative">
             <div className="flex items-center gap-2 font-bold text-gray-800"><Activity className="text-[#3B82F6]" size={18}/> IF Studio</div>
             {step === 'edit' && (
                 <label className="p-2 bg-gray-50 text-gray-600 rounded-lg cursor-pointer"><Plus size={18}/><input type="file" className="hidden" accept="image/*" onChange={handleImageUpload} /></label>
             )}
        </div>
        ...
        >
            ...
            <div className="relative shadow-2xl border border-gray-200 max-w-full max-h-full flex items-center justify-center bg-white/0"> {/* bg-white/0 ensures transparency shows through */}
                <canvas ref={canvasRef} className="max-w-full max-h-full object-contain" style={{ maxHeight: step==='crop' ? '60vh' : 'calc(100vh - 240px)', maxWidth: 'calc(100vw - 20px)' }} />
                
                {/* Mobile Crop UI */}
                {step === 'crop' && (
                     <div className="md:hidden absolute bottom-4 left-4 right-4 bg-white p-4 rounded-2xl shadow-2xl border border-gray-200 animate-in slide-in-from-bottom-10 z-50">
                         <div className="flex justify-between items-center mb-4">
                             <span className="text-xs font-bold text-gray-500 uppercase">Crop Shape</span>
                             <div className="flex gap-2">
                                 <button onClick={() => setFrameShape('circle')} className={`p-2 rounded-lg border ${frameShape === 'circle' ? 'bg-blue-50 border-blue-400 text-blue-600' : 'bg-gray-50'}`}><Circle size={18}/></button>
                                 <button onClick={() => setFrameShape('square')} className={`p-2 rounded-lg border ${frameShape === 'square' ? 'bg-blue-50 border-blue-400 text-blue-600' : 'bg-gray-50'}`}><Square size={18}/></button>
                             </div>
                         </div>
                         <div className="space-y-3 mb-4">
                             <Slider label="Zoom" value={scale} min={0.5} max={3.0} step={0.1} onChange={setScale} />
                         </div>
                         <button onClick={() => setStep('edit')} className="w-full py-3 bg-[#3B82F6] text-white rounded-xl font-bold">Done</button>
                     </div>
                )}
                ...
                {step === 'edit' && (
                     <>
                        ...
                        {/* Quick Crop Button */}
                         <button 
                            className="absolute top-4 left-4 bg-white hover:bg-gray-50 text-gray-600 p-3 rounded-full border border-gray-200 shadow-lg transition-all active:scale-95 z-50 group"
        ...
        {step === 'edit' && (
            <div className="md:hidden bg-white border-t border-gray-200 shrink-0 z-50 pb-safe">
                {/* Active Tool Panel */}
                {activeTab && (
                    <div className="border-b border-gray-100 p-3 bg-gray-50/95 backdrop-blur-xl max-h-[35vh] overflow-y-auto shadow-inner animate-in slide-in-from-bottom-10">
                        <div className="flex justify-between items-center mb-4">
                            <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">{activeTab} controls</span>
                            <button onClick={() => setActiveTab(null)} className="p-1 text-gray-400 hover:text-gray-600"><X size={16}/></button>
                        </div>
                        {renderControls(activeTab)}
                    </div>
                )}
                {/* Tab Bar */}
                <div className="flex justify-around items-center h-12 bg-white">
                    <button onClick={() => handleMobileTabClick('pattern')} className={`flex flex-col items-center gap-1 p-2 w-16 transition-colors ${activeTab === 'pattern' ? 'text-[#3B82F6]' : 'text-gray-400'}`}><Layers size={18}/><span className="text-[8px] font-medium">Pattern</span></button>
                    <button onClick={() => handleMobileTabClick('tune')} className={`flex flex-col items-center gap-1 p-2 w-16 transition-colors ${activeTab === 'tune' ? 'text-[#3B82F6]' : 'text-gray-400'}`}><Move size={18}/><span className="text-[8px] font-medium">Tune</span></button>
                    <button onClick={() => handleMobileTabClick('color')} className={`flex flex-col items-center gap-1 p-2 w-16 transition-colors ${activeTab === 'color' ? 'text-[#3B82F6]' : 'text-gray-400'}`}><Palette size={18}/><span className="text-[8px] font-medium">Color</span></button>
                    <button onClick={() => handleMobileTabClick('print')} className={`flex flex-col items-center gap-1 p-2 w-16 transition-colors ${activeTab === 'print' ? 'text-[#3B82F6]' : 'text-gray-400'}`}><Printer size={18}/><span className="text-[8px] font-medium">Print</span></button>
                </div>
            </div>
        )}
